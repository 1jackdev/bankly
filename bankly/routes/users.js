...
router.get(
  ...
  async function (req, res, next) {
    try {
      // FIXES BUG #2
      if (req.params.username === req.curr_username) {
        let user = await User.get(req.params.username);
        return res.json({ user });
      } else {
        throw new ExpressError(
          "Only  that user or admin can view a user.",
          401
        );
      }
    } catch (err) {
...
router.patch(
  ...
  async function (req, res, next) {
    try {
      ...
      // only allow admins to edit the admin field
      if (!req.curr_admin && fields.admin) {
        throw new ExpressError("Only admins can edit admin status", 401);
      }

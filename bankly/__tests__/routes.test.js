...
describe("POST /auth/register", function () {
  ...
  //TESTS FIX #3
  test("should allow admin user to be registered", async function () {
    const response = await request(app).post("/auth/register").send({
      username: "admin1",
      password: "pwd1",
    ...
    let { admin } = jwt.verify(response.body.token, SECRET_KEY);
    expect(response.statusCode).toBe(201);
...
describe("POST /auth/login", function () {
  ...
  //TESTS BUG #5
  test("should set req.curr_admin to true if warranted", async function (){
    const response = await request(app).post("/auth/login").send({
      username: "u3",
      password: "pwd3",
    });
...
describe("GET /users", function () {
  ...
  // TESTS BUG #1
  test("return only username, first and last names", async () => {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    const user1 = response.body.users[0];
    expect(response.statusCode).toBe(200);
...
describe("GET /users/[username]", function () {
  ...
  // TESTS BUG #2
  test("should not return data on u1 to u2", async function () {
    const response = await request(app)
      .get("/users/u1")
    ...
    expect(response.body).toEqual({
      status: 401,
      message: "Only  that user or admin can view a user.",
    });
  });
});
